...
export async function POST(req: NextRequest) {
  ...
  try {    const payload = await getSafeJsonBody(req);
    ...
    const favourRecieved = await db.favourReceived.create({
      data: {
    ...
    revalidatePath('/');
    return NextResponse.json(favourRecieved, { status: 201 });
  } catch (error) {
    if (error instanceof RequestValidationError) {
      return NextResponse.json({ error: error.message }, { status: 400 });
    }
    console.error(error);
    return NextResponse.json({ error: "Failed to create favour recieved" }, { status: 500 });
  }
}
