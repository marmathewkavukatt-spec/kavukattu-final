...
export default function AddCategoryPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!response.ok) {
        setMessage({ text: data.error || "Save failed. Please try again.", type: "error" });
        return;
      }
      ...
      setMessage({ text: "Save failed. Please check your connection and try again.", type: "error" });
    } finally {
      setSaving(false);
    }
  }
  ...
  async function handleRemoveCoverImage() {
    ...
    try {
      ...
      resetCoverImageInput();
      setMessage({ text: "Cover image removed.", type: "success" });
    } catch {
  ...
  return (
    <div className="pb-8">
      <BackButton />
      <h1 className="mt-4 font-serif text-[36px] sm:text-[40px] lg:text-[48px] font-bold text-stone-800">Add Category</h1>
