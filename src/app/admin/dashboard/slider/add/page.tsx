...
export default function AddSliderPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (res.ok) {
        setMessage({ text: "New slide added successfully.", type: "success" });
        setTimeout(() => router.push("/admin/dashboard/slider/view"), 1500);
      } else {
        setMessage({ text: "Failed to add slide. Please try again.", type: "error" });
      }
    } catch (error) {
  ...
  return (
    <div className="pb-8">
      <BackButton />
      <h1 className="mt-4 font-serif text-[36px] sm:text-[40px] lg:text-[48px] font-bold text-stone-800">Add Slider</h1>
      <p className="mt-2 text-base text-stone-600">Create a new slide for the home page hero section.</p>
