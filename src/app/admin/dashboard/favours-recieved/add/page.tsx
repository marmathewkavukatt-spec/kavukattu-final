...
export default function AddFavourRecievedPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setMessage(null);
    try {
      const res = await fetch("/api/favours-recieved", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(form),
      });
      if (res.ok) {
        setMessage({ text: "Favour Recieved added successfully.", type: "success" });
        setTimeout(() => router.push("/admin/dashboard/favours-recieved/view"), 1500);
      } else {
        setMessage({ text: "Failed to add favour recieved. Please try again.", type: "error" });
      }
    } catch (error) {
  ...
  return (
    <div className="pb-8">
      <BackButton />
      <h1 className="mt-4 font-serif text-[36px] sm:text-[40px] lg:text-[48px] font-bold text-stone-800">Add Favour Recieved</h1>
      <p className="mt-2 text-base text-stone-600">Create a new favour recieved.</p>
      ...
      <form onSubmit={handleSubmit} className="mt-8 space-y-6 rounded-2xl border border-stone-200 bg-white p-8 shadow-lg">
        ...
        <div className="flex flex-col gap-3 pt-2 sm:flex-row">
          ...
          >
            {saving ? "Saving…" : "Add Favour Recieved"}
          </button>
          <button 
            type="button" 
