...
export default function ViewFavoursRecievedPage() {
  const [items, setItems] = useState<FavourRecieved[]>([]);
  ...
  async function load() {
    const res = await fetch("/api/favours-recieved/all");
  ...
  async function handleDelete(id: string) {
    if (!confirm("Delete this favour recieved?")) return;
    await fetch(`/api/favours-recieved/${id}`, { method: "DELETE" });
    setMessage({ text: "Favour Recieved deleted.", type: "success" });
    load();
  }
  ...
  async function handleBulkDelete() {
    if (selectedIds.size === 0) {
      setMessage({ text: "No favours recieved selected.", type: "error" });
      return;
    }

    if (!confirm(`Delete ${selectedIds.size} favour(s) recieved?`)) return;

    setBulkDeleting(true);
    setDeleteProgress({ deleted: 0, total: selectedIds.size });

    try {
      const response = await fetch("/api/favours-recieved/bulk-delete", {
        method: "POST",
      ...
      if (!response.ok) {
        throw new Error("Failed to delete favours recieved");
      }
      ...
      setMessage({ text: `Successfully deleted ${selectedIds.size} favour(s) recieved.`, type: "success" });
      setSelectedIds(new Set());
      load();
    } catch (error) {
      console.error(error);
      setMessage({ text: "Failed to delete some favours recieved.", type: "error" });
    } finally {
  ...
  return (
    <div className="pb-8">
      ...
      <div className="mt-4 flex items-center justify-between">
        <h1 className="font-serif text-[36px] sm:text-[40px] lg:text-[48px] font-bold text-stone-800">View Favours Recieved</h1>
        <Link
          href="/admin/dashboard/favours-recieved/add"
      ...
      <div className="mt-8">
        <div className="flex items-center justify-between mb-6">
          <h2 className="font-serif text-[24px] sm:text-[26px] lg:text-[28px] font-semibold text-stone-800">All Favours Recieved</h2>
          <div className="flex items-center gap-3">
            ...
            <div className="text-sm text-stone-600">
              {items.length} {items.length === 1 ? 'favour recieved' : 'favours recieved'}
            </div>
          </div>
        </div>
        ...
        <ul className="space-y-4">
          {items.length > 0 ? (
            items.map((exp) => {
              const isSelected = selectedIds.has(exp._id);
              return (
                <li 
                  key={exp._id} 
                  ...
                  <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                    <div className="flex items-start gap-3 flex-1">
                      <input
                        type="checkbox"
                        checked={isSelected}
                        onChange={() => toggleSelection(exp._id)}
                      ...
                      <div className="flex-1">
                        {exp.authorImage && (
                          <img 
                            src={exp.authorImage} 
                            alt={exp.authorName} 
                            className="mb-3 h-16 w-16 rounded-full object-cover shadow-md ring-2 ring-stone-200" 
                          />
                        )}
                        <h3 className="font-serif font-semibold text-stone-900">{exp.authorName}</h3>
                        <p className="mt-2 text-sm text-stone-600 line-clamp-3">{exp.content}</p>
                        <div className="mt-3 flex items-center gap-3">
                          <span className="text-xs text-stone-500">Order: {exp.order}</span>
                          {exp.active && (
                    ...
                    <div className="flex w-full flex-col gap-2 sm:ml-4 sm:w-auto sm:flex-row">
                      <Link
                        href={`/admin/dashboard/favours-recieved/edit/${exp._id}`}
            ...
            <li className="rounded-xl border-2 border-dashed border-stone-200 bg-stone-50 p-12 text-center">
              ...
              <p className="mt-4 text-sm font-medium text-stone-600">No favours recieved yet.</p>
              <Link
                href="/admin/dashboard/favours-recieved/add"
