...
export default function EditFavourRecievedPage() {
  ...
  useEffect(() => {
    async function loadFavourRecieved() {
      try {
        const res = await fetch(`/api/favours-recieved/${id}`);
        if (res.ok) {
          ...
          setOriginalImageUrl(data.authorImage || null);
        } else {
          setMessage({ text: "Failed to load favour recieved.", type: "error" });
        }
      } catch (error) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (res.ok) {
        setMessage({ text: "Favour Recieved updated successfully.", type: "success" });
        setTimeout(() => router.push("/admin/dashboard/favours-recieved/view"), 1500);
      } else {
        setMessage({ text: "Failed to update favour recieved. Please try again.", type: "error" });
      }
    } catch (error) {
      console.error("Failed to save:", error);
      setMessage({ text: "Failed to save favour recieved. Please try again.", type: "error" });
    } finally {
      setSaving(false);
    }
  }
    ...
    return (
      <div className="pb-8">
        ...
        <div className="mt-4 space-y-4">
          <div className="h-12 w-64 rounded-full bg-stone-200 animate-pulse" />
          <div className="h-4 w-96 rounded-full bg-stone-200 animate-pulse" />
          <div className="mt-8 space-y-6 rounded-2xl border border-stone-200 bg-white p-8 shadow-lg">
            <div className="h-10 w-full rounded-xl bg-stone-200 animate-pulse" />
            <div className="h-32 w-full rounded-xl bg-stone-200 animate-pulse" />
  ...
  return (
    <div className="pb-8">
      <BackButton />
      <h1 className="mt-4 font-serif text-[36px] sm:text-[40px] lg:text-[48px] font-bold text-stone-800">Edit Favour Recieved</h1>
      <p className="mt-2 text-base text-stone-600">Update the favour recieved.</p>
      ...
      <form onSubmit={handleSubmit} className="mt-8 space-y-6 rounded-2xl border border-stone-200 bg-white p-8 shadow-lg">
        ...
        <div className="space-y-2">
          <label className="block text-sm font-semibold text-stone-700">Favour Recieved Content *</label>
          <textarea 
            value={form.content} 
            ...
            placeholder="Enter the favour recieved content"
          />
        </div>
        ...
        <div className="flex flex-col gap-3 pt-2 sm:flex-row">
          ...
          >
            {saving ? "Saving…" : "Update Favour Recieved"}
          </button>
          <button 
            type="button" 
            onClick={() => router.push("/admin/dashboard/favours-recieved/view")} 
