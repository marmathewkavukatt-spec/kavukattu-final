...
export default function ViewExperiencesPage() {
  ...
  async function handleBulkDelete() {
    if (selectedIds.size === 0) {
      setMessage({ text: "No experiences selected.", type: "error" });
      return;
    }

    if (!confirm(`Delete ${selectedIds.size} experience(s)?`)) return;

    setBulkDeleting(true);
    setDeleteProgress({ deleted: 0, total: selectedIds.size });

    try {
      const response = await fetch("/api/experiences/bulk-delete", {
        method: "POST",
      ...
      setMessage({ text: `Successfully deleted ${selectedIds.size} experience(s).`, type: "success" });
      setSelectedIds(new Set());
      load();
    } catch (error) {
      console.error(error);
      setMessage({ text: "Failed to delete some experiences.", type: "error" });
    } finally {
  ...
  return (
    <div className="pb-8">
      ...
      <div className="mt-4 flex items-center justify-between">
        <h1 className="font-serif text-[36px] sm:text-[40px] lg:text-[48px] font-bold text-stone-800">View Experiences</h1>
        <Link
          href="/admin/dashboard/experiences/add"
      ...
      <div className="mt-8">
        <div className="flex items-center justify-between mb-6">
          <h2 className="font-serif text-[24px] sm:text-[26px] lg:text-[28px] font-semibold text-stone-800">All Experiences</h2>
        ...
        <ul className="space-y-4">
          {items.length > 0 ? (
                ...
                >
                  <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                    ...
                    <div className="flex w-full flex-col gap-2 sm:ml-4 sm:w-auto sm:flex-row">
                      <Link
                        href={`/admin/dashboard/experiences/edit/${exp._id}`}
            ...
            <li className="rounded-xl border-2 border-dashed border-stone-200 bg-stone-50 p-12 text-center">
              ...
              <p className="mt-4 text-sm font-medium text-stone-600">No experiences yet.</p>
              <Link
                href="/admin/dashboard/experiences/add"
